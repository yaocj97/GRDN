...
if __name__ == "__main__":
    ...
    np.random.seed(0)
    all_date = data["train_data_x"].transpose(2, 0, 1)[:, :, :, np.newaxis]
    all_lable = data["train_data_y"].flatten()

    data = shuffle_data(all_date, all_lable)

    train_data_x = np.array(data[0][:-780])
    train_data_y = np.array(data[1][:-780])

    test_data_x = np.array(data[0][-780:])
    test_data_y = np.array(data[1][-780:])

    img_input = Input(shape=(16, 32, 1))
    ...
    plot_model(model,to_file='CNN-3.png')
    #modify
    checkpoint = ModelCheckpoint('CNN-3.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='min')
    tb_callback = TensorBoard(log_dir='./CNN-3',histogram_freq=0,write_images=False, write_graph=True)
